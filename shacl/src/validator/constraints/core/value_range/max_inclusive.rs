...
/// `sh:MaxInclusive` value-range constraint.
pub(crate) struct MaxInclusive<'a>(pub &'a ConcreteLiteral);

impl<S: NeighsRDF + Debug> ConstraintComponent<S> for MaxInclusive<'_> {
    ...
    fn check<E: Engine<S>>(&self, vn: &S::Term, _cx: &mut CheckCtx<'_, S, E>) -> Result<Check, ValidationError> {
        let violates = match S::term_as_sliteral(vn) {
            Ok(lit) => lit.sparql_compare(self.0).map(|o| o.is_gt()).unwrap_or(true),
            Err(_) => true,
        };
    ...
    fn message(&self, _schema: &IRSchema) -> String {
        format!("MaxInclusive({}) not satisfied", self.0)
    }
