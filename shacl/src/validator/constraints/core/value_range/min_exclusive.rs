...
impl<S: NeighsRDF + Debug> NativeValidator<S> for MinExclusive {
    fn validate_native<E: Engine<S>>(
        ...
        validate_with(
            ...
            |n| match S::term_as_sliteral(n) {
                Ok(lit) => lit.sparql_compare(self.min_exclusive()).map(|o| o.is_le()).unwrap_or(true),
                Err(_) => true,
            },
...
#[cfg(feature = "sparql")]
impl<S: QueryRDF + Debug> BasicSparqlValidator<S> for MinExclusive {
    fn validate_sparql(
        &self,
        component: &IRComponent,
        shape: &IRShape,
